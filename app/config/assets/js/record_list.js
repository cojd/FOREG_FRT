    ...
    <div class="card">
      <div class="card-body">
        ...
        <div class="form-group">
          <textarea class="form-control" rows="1" readonly disabled>${plot}</textarea>
        </div>
      </div>
    </div>
    `;
...
function appendTreeItem(row, resultSet, tableId) {
  ...
  let dbh = resultSet.getData(row, 'dbh');
  ...
  Utils.get_tree_prev(stand, plot, tag, function(prev) {
    ...
    <div class="card-body">
    <h5>${tag} | <span class="badge badge-primary">${stand}</span> <span class="badge badge-success">${dbh}</span></h5>
